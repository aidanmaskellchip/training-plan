...
func TestGetDayByIndex(t *testing.T) {
	...
	aw := ActivityWeek{
		Mon: valueobjects.Activity{Distance: 1.0},
		Tue: valueobjects.Activity{Distance: 2.0},
		Wed: valueobjects.Activity{Distance: 3.0},
		Thu: valueobjects.Activity{Distance: 4.0},
		Fri: valueobjects.Activity{Distance: 5.0},
		Sat: valueobjects.Activity{Distance: 6.0},
		Sun: valueobjects.Activity{Distance: 7.0},
	}

	tests := []struct {
		name      string
		index     int
		expected  *valueobjects.Activity
		expectErr bool
	}{
...
func TestGetEasyRunDay(t *testing.T) {
	...
	tests := []struct {
		name        string
		runningDays valueobjects.RunningDays
		longRunDay  int
		expectedDay int
		expectErr   bool
	}{
		{
			...
			runningDays: valueobjects.RunningDays{Days: []int{1, 1, 1, 1, 1, 1, 1}}, // All running days
		...
		},
		{
			...
			runningDays: valueobjects.RunningDays{Days: []int{0, 0, 0, 0, 0, 0, 0}}, // No running days
		...
		},
		{
			...
			runningDays: valueobjects.RunningDays{Days: []int{0, 0, 0, 1, 0, 0, 0}}, // Only Thursday is a running day
